...
`5c5379e`..`de0c8b1` — see "Where we are" above and ADR-019). It touches only
`apps/web/src/server/flags.ts`, `apps/web/src/server/ai/{modelSelection,
simulatedModel}.ts`, `handleAiRequest.ts`, the `.well-known` discovery route,
and this documentation — nothing in Phases 0-9 below. **Phase 3 is still
exactly where work resumes.**
