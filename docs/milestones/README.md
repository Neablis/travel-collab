...
| M18b | Tag focus | **Approved 2026-08-29; placed the same day, immediately after M16. Built 2026-08-30 — gate NOT closed, no flag flipped**, because the checklist's trigger is a *deployed* demo and nothing unattended reaches a protected preview yet; the six behaviours are green on `test:e2e:ci-like`. Carved out of M18's gate on the same terms M11b left M11's: M18 lands both fields, every surface that reads `kind`, and tag chips that render and can be set — M18b lands the behaviour the chips drive. SPEC §11's focus dims off-tag stops to 32% across Timeline, Day columns, Calendar and Map (Calendar instead shows `N of M match`, dimming a no-match card to 0.28). It was the only part of M18 needing shared state above the lens switch, its Calendar rule is a second design, and no M18 gate box measured it. Scope and exit gate are already written, so unlike M11b it needs only a place: `M18b-tag-focus.md` |
...
Current milestone: **M18b — Tag focus** (`M18b-tag-focus.md`) — **still
current**: its work is built and reviewed but its gate has not closed, and this
line moves only when it does. Order from here:
`M18b → M17 → M12 → M13 → M14 → M9`.
...
amendments are recorded in `M17-account-customization.md`. **M11b Playbooks
stays unplaced** — unlike these two it has no scope and no exit gate written,
