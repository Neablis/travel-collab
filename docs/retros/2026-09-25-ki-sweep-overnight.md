...
| *CI root cause* — KI-5 flush | FIXED | on Chromium 151 (CI's browser) the in-flight head's fetch is cancelled just before `pagehide`; the sender treated that transport failure as "not applied", so the flush re-sent a head the server **had** applied and the atomic batch was refused (`400 day-already-exists`) — losing the 3 edits behind it. A head that failed with **no response** now counts as sent. Reproduced by downloading CI's exact Chromium 151 (local is 141 — filed **KI-2026-09-25-i**); full suite red → 183/183 green twice on 151; unit test red first. |
| KI-2026-09-05-l | RESOLVED (contracts tests only) | frozen golden fixture of the oldest stored `SavedStop` (`cccbac8`) + a test that lists any new top-level field that is neither optional nor defaulted. Reproduced: a scratch required field made a pre-M23 stored day vanish from `listSavedDays` (`expected [] to deeply equal [ 'Old day' ]`) with no contract test red; with the guard, both new tests red; `.default()` turns them green. Full `pnpm check` green. |
...
- **My first two CI diagnoses were wrong** (the stream-seq race). The server retry that came from it stands on its own integration test, but was not this failure. The real cause was only visible on CI's browser version; local runs used Chromium 141 under CI's revision number (KI-2026-09-25-i). Trade-off of the fix: if a head's request truly never left the browser, the flush now saves the units behind it without it — KI-5's existing "not a prefix" residual, closed by the same idempotency key.
- **KI-2026-09-05-l: a golden guard, no `version` wrapper on kept days** — additive changes are covered by `.default()`, and a change a default can't express now fails the guard, which is the moment to build the wrapper (ADR-048 already chose this; dated note added there). Rejected: `v: 1` + upcaster now (nothing to migrate, needs a backfill, overlaps KI-2026-09-05-j); relaxing the read boundary (undoes KI-71's fail-closed read). Limit: a *tightened* existing field is caught only if the fixture's values break it.
