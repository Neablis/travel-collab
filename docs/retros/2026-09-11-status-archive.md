# STATUS archive — the narrative moved out of `docs/STATUS.md`, 2026-09-11

The second time this was needed. `docs/STATUS.md` is the file every session is
told to read first; the 2026-08-28 archive cut it from **1,779 lines** and set
the rule that has applied since — *at gate close, a phase's narrative moves to
its milestone file or a retro in the same commit, and this file keeps the
pointer.* By 2026-09-11 it had drifted back to **1,418 lines**, and its "Next
action" section still named M17 as the current work on the day M17's gate
closed. Length was the symptom; **a stale first-read file is the defect.**

**Nothing here was deleted; it was moved.** This is the verbatim content of
`docs/STATUS.md` lines 243-1188 as of commit `845fc48`, in its original order —
the three historical sections, `## Before this` through the builder half. Every
claim in it was true when written and is dated; **none of it is current state.**
For that, read `docs/STATUS.md`.

Where a part has a better home, that home is authoritative and this is the long
form:

| This file's content | The durable home |
|---|---|
| The assistant on the phone, 2026-09-05 (SPEC §23, the `Ask` bottom sheet) | `docs/milestones/M14-rich-layer.md`, and SPEC §23 |
| M14's builder half — ADR-036/037/038, links 2-9, the widget module contract | `docs/milestones/M14-rich-layer.md` |
| The four measured findings a session must not re-derive (ADR-038 decision 4's replaced criterion, and the rest) | ADR-036, ADR-037, ADR-038 |
| The draft-durability decision — one clock, not two | ADR-036's accepted decision and its rejected alternatives |
| The three-PR stack that closed M11a and M11b, 2026-08-30 | `docs/milestones/M11-sharing-and-invites.md`; the stack has merged and both gates closed |
| `KI-2026-09-03-d`'s measurement and resolution | `docs/known-issues/resolved/` |

---

## Moved out of "Where the work is right now", 2026-09-11

These were stacked under the live heading and had stopped being live. **Three
facts in here are still instruction and were kept in `docs/STATUS.md` rather
than archived** — migration `0018` is not applied to production, the `ai-live`
dashboard fallthrough must stay "Simulated", and e2e refuses to start unless
`AI_LIVE=false`. Everything else is the dated narrative.
...
## The "Next action" section as it stood, superseded 2026-09-11
