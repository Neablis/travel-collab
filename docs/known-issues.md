...
- **Area:** `apps/web/src/server/access/shares.ts` (`readFeaturedShare`, deleted), `apps/web/src/app/api/shares/featured/route.ts`, new `apps/web/src/server/demoTrip.ts`
- **Symptom (as filed):** `/welcome`'s "Look around a real trip" and "See a finished one" both link to `/s/featured`, which resolved `DEMO_SHARE_TOKEN`; unset, `SharedTripScreen` rendered **"Nothing to see here / No trip is published here yet."** Unset was not an edge case — the var was not in `.env.example`, neither seeder ever created a share, and a hand-made token died at the next `db:reseed`. CI's own e2e asserted the empty state, so the suite was green *because* nothing was configured.
- **Fix (2026-08-28, ADR-031):** not the fix path this entry proposed. Mitchell rejected a committed world-readable token on two grounds the entry had not weighed — a preview branch would still need a manual step to validate its own front door, and `/s/featured` is an unauthenticated page anyone can hit as often as they like, so a share lookup plus a **full 74-event replay** per view is a load profile set by strangers.
  Instead the demo trip stopped being a row. `server/demoTrip.ts` folds `@tc/fixtures`' `japanTripCommands` through the real domain (`decideTripCommand` → `evolveTrip` → `tripDetailFromState`) in memory and serves it through the same `toSharedView`, the same `SharedTripView` contract, the same route shape and the same page component as any real share. `DEMO_SHARE_TOKEN` and `readFeaturedShare` are deleted.
- **"No database" is enforced, not asserted:** `toSharedView` moved into its own pure module (`access/sharedView.ts`) because `access/shares.ts` imports `db/client`, which constructs a `pg.Pool` at module load — and `demoTrip.test.ts` / `route.test.ts` mock `pg` so that any future import of the client, direct or transitive, fails the test instead of opening a connection on a public path.
- **Proof:** the e2e that used to assert the empty state now asserts the trip — 14 days, "Land at Haneda", "Read only", and the clone button sending a signed-out visitor to `/signin?callbackUrl=%2Fs%2Ffeatured`. 8 new unit tests (fold shape, contract parse, dropped public fields, memoisation, synthetic ids, always-upcoming dates) and 4 integration tests for the clone.
- **Also closed:** "Make this my trip" now works from the demo, through the same `cloneFrom` every other copy uses — the conversion the demo exists for, and the only write anywhere near it.
- **Found by:** Mitchell, 2026-08-28 — "why the homepage preview (see a planned trip) of this trip using the seed data to power it shows Nothing to see here" — and the shape of the fix, 2026-08-28: "it shouldn't use a ENV variable to point to a real DB trip."
- **Cross-reference:** ADR-031 (the decision), ADR-027 (whose "Look around a real trip" section this supersedes, and which predicted this failure in writing), ADR-030 (the fixture the demo now folds), M12 Community (still owns real discovery, and can take `featured` back over).
