...
- **It predates the `test.projects` migration.** Verified against the *unmigrated* config with a probe setup file (`[[WORKER]] process.env.BASE_URL= "/"`), so this is not migration fallout. The migration only made it *visible*: `projects` re-evaluates the config file per project, and `new JSDOM({ url: "/" })` throws `Invalid URL: /` where the old single-config form silently swallowed it.
- **Worked around, not fixed:** `vitest.unit.config.ts` falls back to `localhost:${WEB_PORT}` unless `BASE_URL` is absolute, with the trap documented at the site. The real fix is in `src/config.ts`, which should not be reading a variable Vite owns.
