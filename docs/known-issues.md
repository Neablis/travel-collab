...
### KI-31 — The Preview registry's orphan guard cannot see a Preview whose only usage is a dead component
- **Severity:** cleanup (a guard that reads as green while not guarding)
- **Area:** `apps/web/src/lib/preview-registry.test.ts`, `apps/web/src/components/trip/AddSavedDayButton.tsx`
- **Symptom:** `preview-registry.test.ts`'s "every registered id is used at least once (no orphans)" test scans every non-test `.tsx` for `<Preview id=...>`, including *component definitions*. `AddSavedDayButton.tsx` self-wraps in `<Preview id="add-saved-day">`, so that registry entry counts as used purely because the file exists — but nothing in the app renders `AddSavedDayButton` any more (M10 Wave 2 Phase 1 moved the action out of `TripHeader`, and Phase 6 put the plan-flow version inside `<Preview id="insert-playbook">` instead). The `add-saved-day` entry is therefore a live orphan the orphan guard is structurally unable to report.
- **Scope:** one entry today, but the mechanism is general — any Preview whose sole usage is its own unrendered component file is invisible to the guard.
- **Why not fixed here:** M10 Wave 2 Phase 6's file (`docs/plans/M10-delta/phase-6-growth.md`, Step 3 item 7) explicitly says to *keep* `AddSavedDayButton.tsx` ("Keep the component file; `EndOfTrip` uses the same `insert-playbook` Preview id"), so deleting the file — the one-line fix — is out of this phase's scope. The other fix, teaching the scanner reachability from a rendered root, is a real piece of work and not presentational.
- **Options, cheapest first:** (a) delete `AddSavedDayButton.tsx` and its test and drop the `add-saved-day` registry entry, once a phase owns that call; (b) have the scanner ignore a file whose own default/named export is imported nowhere; (c) leave it and accept the guard is best-effort, documented here.
...
### KI-33 — `UnscheduledRack.tsx` and `unscheduledRack.ts` collide on a case-insensitive filesystem, breaking two test files AND `next build` locally
- **Severity:** reliability (25 unit tests fail and `pnpm --filter web build` fails outright on macOS/Windows; CI unaffected)
- **Area:** `apps/web/src/components/trip/UnscheduledRack.tsx` (the drawer component), `apps/web/src/components/trip/unscheduledRack.ts` (`fitIntoDay`), both from M10 Wave 2 Phase 3
- **Symptom:** `apps/web/src/components/board/TripBoardScreen.tsx` imports the component via `@/components/trip/UnscheduledRack`. On a case-insensitive filesystem (default macOS APFS, Windows NTFS), TypeScript's/webpack's/Vite's module resolution can match this specifier against `unscheduledRack.ts` instead of `UnscheduledRack.tsx` once both are in the module graph (the latter is pulled in independently by `unscheduledRack.test.ts` and `ActivityEditor.tsx`'s `fitIntoDay` import) — the resolved module then has no `UnscheduledRack` export, so `TripBoardScreen` renders `undefined` where the drawer should be.
  - `pnpm --filter web test` (vitest): breaks `TripBoardScreen.test.tsx` (18 tests, `Element type is invalid: ...got: undefined`) and `UnscheduledRack.test.tsx` (7 tests) — 25 of the 665 unit tests.
  - `pnpm --filter web typecheck` (tsc): `TS1149: File name '.../UnscheduledRack.ts' differs from already included file name '.../unscheduledRack.ts' only in casing.`
  - `pnpm --filter web build` (Next.js/webpack): **fails outright** — the same casing warning during compilation, then `Failed to compile` on the type-check pass with the same `TS2305`/no-export error, so **no production build can be produced on this filesystem at all**. This also means `pnpm --filter web test:e2e:ci-like` (which runs `pnpm build` first) cannot run locally on a case-insensitive machine, full stop — not degraded, blocked.
  - Confirmed identical on a clean checkout of `main` at `624a0db` via a throwaway worktree — present before Phase 7 and not introduced by it.
- **Scope:** local/dev-machine only. **CI is not affected** — GitHub Actions runners use a case-sensitive filesystem (ext4), where `UnscheduledRack` can only ever resolve to `UnscheduledRack.tsx`; CI remains the authoritative signal, same reasoning as KI-27/KI-32. `next dev` (Turbopack, dev mode) does **not** hit the build-time check and was used for this phase's manual verification instead — see the Phase 7 PR.
- **Why not fixed here:** the fix is a rename of one of the two files (e.g. the logic module to `fitIntoDay.ts`, matching what it exports, or the component to something like `UnscheduledRackDrawer.tsx`) — a real, if small, change to Phase 3's files, which is out of Phase 7's presentational-only, don't-touch-other-phases'-files scope. `ActivityEditor.tsx` (Phase 7 Task 7.1) now also imports `fitIntoDay` from `unscheduledRack.ts`, so whichever phase picks this up should re-check that import too. Given this now blocks *building the app at all* on a common dev OS, not just two test files, this should be picked up sooner than a typical cosmetic/cleanup KI — flagging for priority at the next gate review rather than leaving it indefinitely.
