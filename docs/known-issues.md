...
### KI-77 — The same history reconcile still clears a queue that fills while the command is IN FLIGHT
...
### KI-78 — `next dev` leaves three files in the working tree that are not ours and are not ignored
...
- **What happens:** the guard is `if pg_isready -q -h localhost -p ${POSTGRES_PORT:-5433} 2>/dev/null; then pnpm test:int; else <print SKIPPED banner>; fi`. `pg_isready` ships with the Postgres **client** tools. Where Postgres runs in Docker and no client is installed on the host — Mitchell's laptop, as of 2026-08-28 — the binary is absent, the shell's "command not found" goes to `/dev/null`, and the non-zero exit is indistinguishable from "no database". The banner prints, `test:int` never runs, and **`pnpm check` still exits 0**.
- **Observed 2026-08-28 (M11's exit-gate run):** `pnpm check` reported `EXIT=0` having printed `SKIPPED: integration tests did NOT run. No Postgres on localhost:5433`. Postgres was up and reachable the entire time, verified two independent ways: `docker exec travel-collab-postgres-1 pg_isready -U postgres` → `accepting connections`, and a `pg` client connect from the host on 5433 running `select 1`. `which pg_isready` → not found. Running `pnpm test:int` directly: **25 files, 242 tests, all green**.
- **Why the banner does not save it:** the banner is honest about what happened and is printed *to stdout of a command that then succeeds*. In a long `pnpm check` log it scrolls past, and the exit code — the thing a person or a hook actually reads — says everything passed. The banner's own last line ("A green `pnpm check` here is NOT a green CI") is true and is exactly the failure mode; it just cannot be enforced by a message.
- **The fix is to probe the way the app does**, not with a tool that need not be installed: a short Node connect against `DATABASE_URL` using the `pg` client the repo already depends on. That also removes the second, quieter bug — the probe hardcodes host `localhost` and reads `POSTGRES_PORT` rather than parsing `DATABASE_URL`, so it is already wrong for anyone whose database is not on `localhost:5433`. Distinguishing "no client tool" from "no database" and failing loudly on the first would be the minimum.
- **Not fixed here:** M11's gate commit is the four status flags plus its own evidence; changing what `pnpm check` runs is a tooling change that deserves its own diff, and the gate did not need it — the integration suite was run directly and is green.
- **First noted:** 2026-08-28 (M11's exit-gate run).
...
- **It detonated, 2026-08-29 — no longer a landmine.** Two agents were working in two worktrees against the one Docker `travel` database. One created a trip through the UI for a browser check, opened it, and got **`not-found`**: `select count(*) from events where stream_id = <that trip>` returned 0, with 34 other trips present and **five `vitest` processes running** in the other worktree. The trip had been created and truncated out from under the page between two clicks. Nothing was flaky and nothing was wrong with the code being checked — the failure was another worktree's `beforeEach`. Worth knowing because the symptom (a freshly created row that is simply gone) reads as a projection bug, which is a long way to chase.
- **The cheap workaround, until the fix path is taken:** give the worktree its own database — `create database travel_<branch>`, point `apps/web/.env.local`'s `DATABASE_URL` at it, `pnpm --filter web db:migrate`. Two minutes, and it makes both the browser check and the integration run honest. A fresh database also sidesteps the local `travel` database's migration drift.
- **First noted:** 2026-08-28 (review-remediation docs pass). **First observed live:** 2026-08-29.
...
- **The barrier's cost is now measured, not argued (2026-08-29).** The M11-fallout KI cluster was asked to fix this one and deliberately did not; the entry's own "which trade-off is right is a product call" is the reason, and AGENTS.md's "pause before a plan-deviating design decision" is the rule. What that pass added is evidence, so the call is cheap to make:
  - The recommended fix is one line in `enqueue` — `const barrier = state.pending.some((u) => u.predictedDetail === null)`, and the new unit takes `predictedDetail: barrier ? null : prediction.detail`.
  - Applied experimentally, **exactly one test changes** across `components/trip/context/`: this entry's own characterization test, failing `expected +0 to be 1` — the newly queued day is no longer on the board at all. Nothing else in the optimistic layer or the provider notices. So the change is as contained as it looks, and its whole cost is the one thing the entry predicted: the user's next edit visibly does nothing, with no error and no explanation.
  - **What the barrier does and does not buy.** It restores the property `confirmHead`'s comment claims — the preview is always a state the send sequence actually passes through (a prefix of it). It does **not** make the preview show the retained work; nothing can, because that work is unpredictable by construction. So the choice is between "the preview is missing an earlier edit" and "the preview is missing your latest one", and only the third option below makes the incompleteness visible either way.
  - **The recommendation, if it is wanted:** take the third option first (state the incompleteness), and only then decide whether the barrier is still worth the silent-click. A barrier without a surface saying why the click did nothing invites the user to click again, which is a duplicate edit.
- **First noted:** 2026-08-28 (CodeRabbit review of PR #73). **Re-examined and left open, deliberately:** 2026-08-29.
