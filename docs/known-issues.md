...
- **Symptom:** `apps/web/src/components/board/TripBoardScreen.tsx` imports the component via `@/components/trip/UnscheduledRack`. On a case-insensitive filesystem (default macOS APFS, Windows NTFS), TypeScript's/webpack's/Vite's module resolution can match this specifier against `unscheduledRack.ts` instead of `UnscheduledRack.tsx` once both are in the module graph (the latter is pulled in independently by `unscheduledRack.test.ts` and `ActivityEditor.tsx`'s `fitIntoDay` import) — the resolved module then has no `UnscheduledRack` export, so `TripBoardScreen` renders `undefined` where the drawer should be.
  ...
  - `pnpm --filter web typecheck` (tsc): `TS1149: File name '.../UnscheduledRack.ts' differs from already included file name '.../unscheduledRack.ts' only in casing.` — quoted verbatim from the actual `tsc` output; the real component file is `UnscheduledRack.tsx`, tsc's own message just omits the `x` (CodeRabbit, PR #32).
