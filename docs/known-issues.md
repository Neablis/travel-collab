...
- **The preview gap is closed, and it was hiding a real defect.** This bullet used to say the preview "was never loaded — it is behind Deployment Protection and no bypass token exists here". That premise was wrong in the same shape as the "no browser here" one it sits next to: a share link mints in one MCP call, and two container-specific obstacles (Chromium not trusting the egress CA; `*.vercel.app` tunnelled with a ClientHello the tunnel cannot carry) accounted for the rest. `pnpm --filter web walk:preview` is the recipe; `docs/guidelines/cloud-agent-sessions.md` has the diagnosis. Walked 2026-08-29 against the `claude/project-review-plan-xnsmw1` preview: `/`, `/welcome`, `/demo` and `/signin` all 200, and **one CSP violation the twenty local surfaces could not produce** — `Refused to load the script 'https://vercel.live/_next-live/feedback/feedback.js'`, the Vercel Toolbar's loader. That broke the Flags Explorer workflow `environments-and-deploys.md` documents for flipping `ai-live` per session, from the day the CSP landed. Fixed the same day, preview-only, gated on `VERCEL_ENV` and pinned by `apps/web/next.config.test.ts`.
- **What the walk still does not cover:** Vercel's own `/_vercel/insights/script.js` and `/_vercel/speed-insights/script.js` under `script-src 'self'` were not seen firing (they 404 locally, and analytics is not enabled on the walked preview). The dev-mode CSP branch (`'unsafe-eval'`, `va.vercel-scripts.com`) was not run. Google OAuth's 302 hand-off against `form-action 'self'` is still reasoning. And `frame-src`/`frame-ancestors`/`object-src`/`base-uri` were never counter-probed — nothing tried to violate them, which is weaker than proving they would block something. One new caveat from the preview lane specifically: a cloud-session walk runs at **TLS 1.2**, so it is not evidence about anything TLS-version-dependent.
