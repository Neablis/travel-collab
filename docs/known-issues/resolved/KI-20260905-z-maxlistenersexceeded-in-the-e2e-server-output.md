### KI-2026-09-05-z — `MaxListenersExceededWarning: 11 close listeners added to [ServerResponse]` appears 299 times in a green e2e run, with no stack captured — DOWNGRADED, traced: the adder is Next 16.3.3's own vendored code, and none of this entry's suspects
...
- **TRACED 2026-09-12 — the entry's own suggested next step, run.** `node --trace-warnings`, on the `ci-like` lane (`CI=true`, `next start`, a real production build), by adding `--trace-warnings` to `NODE_OPTIONS` and invoking the wrapper directly. Seven warnings in a two-test run of `m1-board.spec.ts`, **every one with an identical stack** (confirmed: exactly one distinct call site across all seven):

  ```
  (node:6233) MaxListenersExceededWarning: Possible EventEmitter memory leak detected.
...
- **Every suspect this entry named is exonerated, by the stack and by grep.** It is not Sentry's request instrumentation and not ours: `apps/web/instrumentation.ts`, `sentry.shared.ts` and `sentry.server.config.ts` contain **zero** occurrences of `.on(`, `addListener` or `ServerResponse` between them. Nothing in this repository appears anywhere in the stack.
- **The mechanism.** Next's bundled HTTP proxy (`httpxy`) pipes an upstream response into the `ServerResponse`, and `Readable.pipe` registers a `close` listener through Next's bundled `compression` middleware on the way. More than ten pipes against one `ServerResponse` crosses Node's default ceiling and Node warns — **once per emitter**, which is the detail that reframes the original observation: 299 warnings are **299 distinct, short-lived responses each crossing ten**, not one long-lived object accumulating 299 listeners. That is the opposite of the unbounded-growth shape the entry reasonably feared. We configure none of this — `apps/web/next.config.ts` declares no `rewrites`, no proxy, no `compress` setting.
- **Disposition: DOWNGRADED, not fixed, and deliberately not worked around.** There is no defect in this repository to fix. The entry's own instruction — *"raising the limit hides the count without answering the question"* — still stands, and now the question is answered, so raising `setMaxListeners` would be pure noise suppression over vendored code we do not own. The honest close is the trace. If the console noise ever becomes worth acting on, the lever is upstream (Next 16.3.3's `httpxy`/`compression` interaction), not here.
- **Still true, and still the reason this was never urgent:** no test fails, no user-visible symptom exists, every run that produced it was green, and on a serverless deploy each invocation is short-lived anyway.
