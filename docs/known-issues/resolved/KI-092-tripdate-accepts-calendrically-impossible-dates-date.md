### KI-92 — `TripDate` accepts calendrically impossible dates, so the date parsers can only reject them by throwing — RESOLVED
...
- **Resolved 2026-09-24 (KI pass).** `SetTripStartDate` and `SetTripDates` now parse dates through `TripDateInput` in `packages/contracts/src/trip.ts` — the `YYYY-MM-DD` regex plus a `.refine` that round-trips the value through `Date.UTC` — so an impossible date is unrepresentable in a command. The stored `TripStartDateSetV1` event keeps the shape-only regex on purpose (replay must never break on history), and a test pins that it still parses `2026-02-30`. **Proof:** five new cases in `packages/contracts/test/trip.test.ts` (`2026-02-30`, `2027-02-29`, `2026-13-45`, `2026-00-10`, `2026-04-31`, each on both commands) failed on HEAD — `× rejects the impossible date 2026-02-30 on both date commands … Tests 5 failed | 22 passed (27)` — and pass after (`Tests 27 passed (27)`), with `2028-02-29` accepted. `openapi.test.ts` passes unchanged (4/4). Full `pnpm check` run as a contracts change requires. CHANGELOG entry: `docs/contracts/CHANGELOG.md`, 2026-09-24. The path-parameter half (non-UUID ids 500ing) was KI-2026-09-05-x, already resolved separately.
