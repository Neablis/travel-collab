...
- **Correction (2026-09-25, PR #234 review):** rejected alternative (b) was wrong, for the reason KI-2026-09-19-c's correction gives. A tab still on the previous bundle requires `sharedDayCount`, so Discover went to its error state until reload. `GET /api/playbooks` now sends `sharedDayCount` beside `sharedPlaybookCount` for one release, via `server/playbookWireAliases.ts`. Pinned by the same int test in `board/route.int.test.ts`, which fails `expected undefined to be 3` with the alias removed.
