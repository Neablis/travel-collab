...
4. ~~**Open sweeps and phase PRs as drafts.** One word in `.claude/commands/ki-sweep.md` step 6a and in `phase-implementer`'s brief, matching `AGENTS.md:236`. Intermediate pushes then cost nothing until `gh pr ready`.~~ Superseded by a hook. `scripts/hooks/draft-pr-guard.mjs` (`73584ec`, #199) asks on any `gh pr create` without `--draft`, and on `gh pr ready` without a Tier-3 stamp. The prose in `ki-sweep.md` step 6a still just says "Open a PR".
...
- **Re-verified 2026-09-25 (overnight sweep):** partly true. **Struck:** fix item 4. Draft-opening is now enforced at the `gh pr create` call by `scripts/hooks/draft-pr-guard.mjs`, which `.claude/settings.json` registers as a `PreToolUse` hook on `Bash(gh *)`. `pnpm check` also now records a Tier-3 stamp (`package.json:28`, `scripts/tier3-stamp.mjs record`), and `gh pr ready` asks when that stamp is missing. **Still holds, with current line numbers:** `check` still runs no e2e (`package.json:28`). Tier 3 still says `test:e2e:ci-like` *"if a user flow changed"*, with no list of change classes (`AGENTS.md:398-399`). `laneReporter` is still registered only when `CI` is unset (`apps/web/playwright.config.ts:32`, was `:11`). With `CI=true`, `timeout` is still 30s (`:53`) and `retries` still 1 (`:108`). `.claude/protocol/adapter.json` still has no CPU or heavy-lane resource, and its `symptom` still excludes `test:e2e`. The batching rule still argues from *"private on a GitHub Free plan (2,000 Linux minutes/month)"* (`AGENTS.md:303-304`, was `:241-245`). No push-time guard reads CodeRabbit's in-flight review (`grep -rl -i coderabbit scripts/hooks/` finds only a comment in `subagent-report-conformance.mjs`). `docs/guidelines/testing.md` does not name the three green-for-the-wrong-reason sub-shapes. The `ci.yml` draft gates have moved to `:106` and `:169`.
