...
- **Re-verified 2026-09-25 (overnight sweep):** still true. All eight bundles still open `bundle.sources` with `PARTIALLY VERIFIED 2026-09-06` and carry `"origin": "ai"` (which becomes `authorKind`, `packages/fixtures/src/bundle/toPlaybooks.ts:103`); 60 days and 415 `cost*`/`price*` fields across the eight (`grep -cE '"(cost|price)[A-Za-z]*":'`, against ~418 above). The only later commits to these files (`cec7b1a` coordinates, `5c41985` countryCode, `b3c6a2a` country codes) touch no price, so no price pass has run.
