...
- **Re-verified 2026-09-25 (overnight sweep):** still true. `apps/web/src/app/api/v1/trips/route.ts` pre-decides both commands (`:88-92`) and then runs `CreateTrip` (`:99`) and the dates command (`:102`) as two `runCommand`s, compensating with `DeleteTrip` on refusal and on throw (`:104`, `:108`). `CreateTrip` (`packages/contracts/src/trip.ts:46-54`) still has no `startDate`/`endDate`, so option B has not been taken.
