...
- **Re-verified 2026-09-25 (overnight sweep):** STILL TRUE. `server/ai/modelSelection.ts:54-56` still returns `{ live: process.env.AI_LIVE === "true", source: "env" }` whenever `AI_LIVE` is set, on Vercel or not; the only Vercel-specific behaviour is `warnIfVercelOverride()` (line 97). `vitest run -c vitest.unit.config.ts src/server/ai/modelSelection.test.ts` → 21/21 pass, including the KI-24 warning block (line 128). The decision (keep or drop the escape hatch) is still Mitchell's and unmade.
