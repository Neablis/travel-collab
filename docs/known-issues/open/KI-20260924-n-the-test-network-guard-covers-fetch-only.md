...
- **Area:** `apps/web/src/test-support/networkGuard.ts`, installed by `apps/web/vitest.setup.ts` (unit) and `networkGuard.setup.ts` (integration, via `vitest.config.ts`).
...
- **Why not fixed here:** checked 2026-09-24. Nothing that tests import uses those transports; every third-party client in `src/server` goes through `fetch`. Sentry's node transport is the exception, and no test initialises Sentry with a real DSN (`NEXT_PUBLIC_SENTRY_DSN` now defaults to `""` in the unit setup). Covering them is cheap only if something starts to need it.
