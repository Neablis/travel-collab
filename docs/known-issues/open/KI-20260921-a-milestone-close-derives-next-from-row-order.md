...
- **Mitigation shipped 2026-09-21, and it is a stopgap, not a fix:** `--next <id>`, which lets the operator state the successor the files cannot. It validates against the index and refuses an unknown id, an already-ticked one, the milestone being closed, and a bare `--next` with no value; it prints a NOTE when it overrides the row-order default. The default behaviour is unchanged, **so the trap is still armed for anyone who does not pass the flag.**
