...
- **First noted:** 2026-09-01 on PR #105, by Mitchell, reading CodeRabbit's
  ...
  against the API.
- **Re-verified 2026-09-25 (overnight sweep):** still true, checked with this entry's own three-part method. The combined status on PR #228 (head `ffb84bd`, 2026-09-25 00:18Z) is `context: CodeRabbit`, `state: success`, `description: "Review skipped: manual review required for this OSS repository"`. For contrast, #231 (head `7e97c25`) shows `Review completed` and a real `coderabbitai[bot]` review (`Plan: Advanced`, *"up to 1 included review per hour"*), which is what the manual, human-triggered step produces. The "required check" wording this entry calls false still survives at `.coderabbit.yaml:33` (*"CodeRabbit stays a required check"*), although the corrections at `.coderabbit.yaml:7-11` contradict it. `AGENTS.md` no longer contains "registered status check", and `docs/STATUS.md` no longer contains the drafts-cause wording.
