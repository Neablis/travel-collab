...
- Assumption recorded: findings are **not** filed as KIs by this review. The register is for things knowingly left unfixed; this list exists to be fixed. Anything Mitchell defers can be moved with a `git mv` and a heading edit — every file already carries the KI fields.
  **Overridden 2026-09-05 by Mitchell**, who asked for the whole list to be cross-checked against the register and entered into it. Done: **fourteen** existing open entries gained a dated "2026-09-05 overnight review" block — twelve of them linking the finding that re-confirmed or extended them (KI-5, 9, 15, 22, 79, 90, 92, 2026-09-02-a/-b/-c/-d, 2026-09-03-b), plus KI-2026-09-05-a and -c, which have no finding file of their own and instead carry stream B's conclusion that they share one cause (the macro node is an inline atom) while -b does not. **23 new entries** `KI-2026-09-05-d` … `-z` cover every one of the 56 finding files plus stream G's `MaxListenersExceededWarning` lead. One finding resolved to an annotation rather than an entry: **F-G02** is a sub-finding of open KI-15 and was written into it, together with F-F12's "the fix exists off the shelf and is unwired" half. **F-D09**'s recommendations that are opportunities rather than defects (automatic Copilot review, skip-job conversion then branch protection, `tsc` three times per PR) were deliberately left out of the register as roadmap material; only its item 11, the PreToolUse test-lane guard that was never built, is filed (in `-m`).
- Why the new entries are bundled rather than one-per-finding: `scripts/check-ki-filenames.mjs` allows a **single lowercase letter** as the same-day discriminator, so a date caps at 27 entries and `a`–`c` were already taken on 2026-09-05. Twenty-three slots, fifty-six findings. Each bundle groups findings that share an insertion point or a fix (B01+B09, C03+D03+C07, E04+F03+E03, C04+F05) or that are XS cleanups of one kind, lists every member with a direct link, and says to strike a line as it lands and resolve when all are struck. Splitting a bundle later costs a `git mv` and a heading edit.
