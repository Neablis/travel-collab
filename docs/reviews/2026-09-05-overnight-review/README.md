...
- Assumption recorded: findings are **not** filed as KIs by this review. The register is for things knowingly left unfixed; this list exists to be fixed. Anything Mitchell defers can be moved with a `git mv` and a heading edit — every file already carries the KI fields.
  **Overridden 2026-09-05 by Mitchell**, who asked for the whole list to be cross-checked against the register and entered into it. Done: **twelve** existing open entries gained a dated "2026-09-05 overnight review" block linking the finding that re-confirmed or extended them (KI-5, 9, 15, 22, 79, 90, 92, 2026-09-02-a/-b/-c/-d, 2026-09-03-b), and **23 new entries** `KI-2026-09-05-d` … `-z` cover every one of the 56 finding files plus stream G's `MaxListenersExceededWarning` lead. Two findings resolved to annotations rather than entries: **F-G02** is a sub-finding of open KI-15 and was written into it (with F-F12's "the fix exists and is unwired" half), and the widget-layer note that KI-2026-09-05-a and -c share one cause went onto both. **F-D09**'s recommendations that are opportunities rather than defects (automatic Copilot review, skip-job conversion then branch protection, `tsc` three times per PR) were deliberately left out of the register as roadmap material; only its item 11, the PreToolUse test-lane guard that was never built, is filed (in `-m`).
