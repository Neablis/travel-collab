...
- [x] The save indicator ships **two** states, not three — **descoped by Mitchell, 2026-08-25**: *"I really dont think we need the third state, just list it as a KI, and we can come back someday to it."* The error state is deferred to **KI-36**, which already records that `failHead` (`optimistic.ts:81-83`) discards the pending queue, so nothing retries and the design's `Couldn't save — retrying` label would be false today. Everything else on this line shipped: `e37cc20` gives the 11px dot, the twin haloes with the 0.7s-delayed second, the `om-save-pulse` keyframes, the saved state's `All changes saved` accessible name, and `prefers-reduced-motion` dropping the haloes while keeping the label. A `@ts-expect-error` assertion pinned `pending` to `boolean | number` so the prop could not quietly grow a third state without this decision being revisited. **Superseded 2026-08-25**: KI-36 was closed via its Option 1 (a failed send now retains its queue and exposes a manual retry), so the third state ships after all — with **`Couldn't save`** plus a Retry button, deliberately not the handoff's `Couldn't save — retrying`, since retry is manual. The pin was removed as part of that close and replaced by tests of the real state. Task 8b.4's banner (the next line) is still not shipped.
