...
describe("block resolvers", () => {
  it("itinerary.day returns the bound day's activities", () => {
    const r = itineraryDay.resolve({ trip: detail, page: ctx }, { dayRef: { kind: "index", index: 0 } });
  ...
  it("itinerary.day reads the day named in its own params", () => {
    const r = itineraryDay.resolve({ trip: twoDays, page: ctx }, { dayRef: { kind: "index", index: 1 } });
    expect(r.status).toBe("ok");
    if (r.status === "ok") {
  ...
  it("itinerary.day is unbound when its params name no day", () => {
    expect(itineraryDay.resolve({ trip: twoDays, page: ctx }, {}).status).toBe("unbound");
  });
  it("itinerary.day is empty for a day with no activities", () => {
    ...
    expect(itineraryDay.resolve({ trip: emptyDay, page: ctx }, { dayRef: { kind: "index", index: 0 } }).status).toBe("empty");
  });
  it("itinerary.trip returns all days; empty when there are none", () => {
    expect(itineraryTrip.resolve({ trip: detail, page: ctx }, {}).status).toBe("ok");
    expect(itineraryTrip.resolve({ trip: { ...detail, days: [] }, page: ctx }, {}).status).toBe("empty");
  });
  it("costs.table lists day + backlog rows with a total; empty when zero", () => {
    const r = costsTable.resolve({ trip: detail, page: ctx }, {});
    expect(r.status).toBe("ok");
    if (r.status === "ok") expect(r.value.total).toBe("$50.00");
    expect(costsTable.resolve({ trip: { ...detail, tripCostTotal: 0, days: [{ dayId: "d0", activityIds: [], date: "2026-08-01", costSubtotal: 0 }] }, page: ctx }, {}).status).toBe("empty");
  });
});
