...
function stateOf(costs: number[], onDay: boolean[]): TripState {
  ...
  costs.forEach((c, i) => {
    const id = `a${i}`;
    activities[id] = { title: `A${i}`, timeWindow: null, location: null, notes: null, anchors: [], kind: "planned" as const, tags: [], cost: c === 0 ? null : { amountMinor: c, currency: "USD" } , bookedBy: null, participants: []};
