...
// The landing CTAs are ordinary links now, not Preview shells — and since
// ADR-031 they land on the built-in demo trip, which is the Japan fixture
// folded in memory. That is what makes this assertable in CI at all: the old
// `DEMO_SHARE_TOKEN` was unset here (and on every preview branch, and in every
// fresh clone), so the only thing this suite could prove was that the front
// door's second-most prominent CTA dead-ended in an empty state (KI-61).
test.describe("the landing page's peek CTAs", () => {
  test.use({ storageState: { cookies: [], origins: [] } });

  test("both open the demo trip, read-only, with no account", async ({ page }) => {
    ...
    // The real fixture, not a placeholder and not an empty state: 14 days,
    // named stops, real money.
    await expect(page.getByRole("heading", { name: "Japan: Tokyo → Kyoto → Osaka", level: 1 })).toBeVisible();
    await expect(page.getByText("Read only")).toBeVisible();
    await expect(page.getByText("14 days")).toBeVisible();
    await expect(page.getByText(/An example trip, to look around/)).toBeVisible();
    await expect(page.getByText("Land at Haneda").first()).toBeVisible();

    // Read-only by construction — nothing on this page can change anything.
    await expect(page.getByRole("button", { name: "Add stop" })).toHaveCount(0);

    // …and the one thing it does offer is the way in. Signed out, that is a
    // trip to /signin and back to this page.
    await page.getByRole("button", { name: "Make this trip mine" }).click();
    await expect(page).toHaveURL(/\/signin\?callbackUrl=%2Fs%2Ffeatured$/);
  });
});
