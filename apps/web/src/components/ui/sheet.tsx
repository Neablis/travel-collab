...
/**
 * `rail` is the ordinary side sheet. `full` fills the window.
 *
 ...
 */
export type SheetSize = "rail" | "full";
  ...
  return (
    <RadixDialog.Root open={open} onOpenChange={onOpenChange}>
      <RadixDialog.Portal>
        ...
        <RadixDialog.Content
          className={cn(
            "overlay-layer fixed inset-y-0 right-0 flex w-full flex-col bg-surface p-5 shadow-overlay",
            // `max-w-measure` is what makes the rail a rail; dropping it for
            // `inset-x-0` is what makes `full` full.
            size === "rail" ? "max-w-measure" : "inset-x-0",
          )}
        >
